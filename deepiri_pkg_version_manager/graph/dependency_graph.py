...
class DependencyGraph:
    ...
    def get_dependencies(self, name: str) -> list[str]:
        ...
        if node_id is None:
            return []
        return [self.get_node_name(succ) for succ in self.graph.successors(node_id)]

    def get_dependents(self, name: str) -> list[str]:
        node_id = self.get_node_id(name)
        if node_id is None:
            return []
        return [self.get_node_name(pred) for pred in self.graph.predecessors(node_id)]
    ...
    def get_all_dependents_recursive(self, name: str) -> set[str]:
        ...
        return {self.get_node_name(n) for n in descendants if self.get_node_name(n)}
    ...
    def get_all_dependencies_recursive(self, name: str) -> set[str]:
        ...
        return {self.get_node_name(n) for n in ancestors if self.get_node_name(n)}
    ...
    def get_leaves(self) -> list[str]:
        ...
        return [self.get_node_name(node) for node in leaves if self.get_node_name(node)]
