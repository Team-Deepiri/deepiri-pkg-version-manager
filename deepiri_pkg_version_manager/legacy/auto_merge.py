...
def validate_pr(pr_url: str, dep_path: str):
    ...
    view_raw = run_command(
        ["gh", "pr", "view", str(number), "--repo", repo_path, "--json", "files"], dep_path
    )
    if view_raw is None:
    ...
    for file in files:
        if file["path"] not in allowed_files:
            logging.error(
                f"[red]Error:[/red] File '{file['path']}' is not allowed in PR '{pr_url}'"
            )
            return False
        elif file["additions"] != 1 or file["deletions"] != 1:
            logging.error(
                f"[red]Error:[/red] File '{file['path']}' has unexpected changes in PR '{pr_url}'"
            )
            return False
        else:
...
def auto_merge(pr_url: str, branch: str, dep_path: str):
    if GITHUB_PAT == "-1":
        logging.error(
            "GitHub PAT is not set, please set it in the .env file as GITHUB_PAT to enable auto-merge"
        )
        return False
    ...
    if not run_command(
        ["gh", "pr", "review", str(number), "--repo", repo_path, "--approve"],
        dep_path,
    ...
    if not run_command(
        ["gh", "pr", "merge", str(number), "--repo", repo_path, "--merge"],
        dep_path,
        env_overrides=github_env,
    ):
