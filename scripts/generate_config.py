...
def parse_gitmodules(repo_name, path):
    ...
    for sub_path, url in matches:
        ...
        add_dependency(
            dep_name, {"repo": f"{ORG}/{repo_name}", "type": "submodule", "path": sub_path}
        )
    ...
    for section in ["dependencies", "devDependencies"]:
        ...
        for dep in deps:
            if dep.startswith("deepiri") or dep.startswith("diri"):
                add_dependency(dep, {"repo": f"{ORG}/{repo_name}", "type": "npm", "package": dep})
...
def parse_pyproject(repo_name, path):
    ...
    for dep in matches:
        add_dependency(dep, {"repo": f"{ORG}/{repo_name}", "type": "python", "package": dep})
